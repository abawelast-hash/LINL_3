...
var RC = {
  ...
  ringOuter:    'rgba(59,130,246,0.18)',
  ringLabel:    'rgba(59,130,246,0.35)',
  cross:        'rgba(59,130,246,0.06)',
  sweepHi:      'rgba(59,130,246,0.18)',
  sweepMid:     'rgba(59,130,246,0.08)',
  sweepLo:      'rgba(59,130,246,0.03)',
  sweepZero:    'rgba(59,130,246,0)',
  sweepLine:    'rgba(59,130,246,0.55)',
  label:        'rgba(148,163,184,0.4)',
  tickMaj:      'rgba(59,130,246,0.3)',
  tickMin:      'rgba(59,130,246,0.12)',
  centerGlow:   'rgba(59,130,246,0.4)',
  insideC:      '#22C55E',
  outsideC:     '#EF4444',
  northFill:    'rgba(239,68,68,0.85)',
  northGlow:    'rgba(239,68,68,0.4)',
};
...
function drawRadar() {
  ...
  if (geoRingPx > 4 && geoRingPx < R) {
    ctx.strokeStyle = inside ? 'rgba(34,197,94,0.25)' : 'rgba(239,68,68,0.2)';
    ctx.lineWidth = 1.5;
    ctx.setLineDash([4, 4]);
    ...
    ctx.fillStyle = inside ? 'rgba(34,197,94,0.5)' : 'rgba(239,68,68,0.4)';
  ...
  if (locReady && userDist !== null) {
    ...
    bg.addColorStop(0, inside ? 'rgba(34,197,94,0.4)' : 'rgba(249,115,22,0.4)');
    bg.addColorStop(1, inside ? 'rgba(34,197,94,0)' : 'rgba(249,115,22,0)');
    ...
    if (branchLogoLoaded) {
      ...
      ctx.strokeStyle = inside ? 'rgba(34,197,94,0.6)' : 'rgba(249,115,22,0.6)';
      ctx.lineWidth = 2;
      ctx.shadowColor = inside ? 'rgba(34,197,94,0.5)' : 'rgba(249,115,22,0.5)';
    ...
    ctx.strokeStyle = inside ? 'rgba(34,197,94,' + (0.25 - bPulse*0.15) + ')' : 'rgba(249,115,22,' + (0.25 - bPulse*0.15) + ')';
    ...
    if (branchDist > 30) {
      ...
      ctx.fillStyle = inside ? 'rgba(34,197,94,0.6)' : 'rgba(249,115,22,0.6)';
  ...
  ctx.strokeStyle = 'rgba(59,130,246,' + (0.25 - cpulse*0.15) + ')';
  ctx.lineWidth = 1; ctx.stroke();
  ...
  // ── Outer ring color ──
  ctx.beginPath(); ctx.arc(cx, cy, R, 0, Math.PI*2);
  var ringColor = inside ? 'rgba(34,197,94,.4)' : (hasLoc ? 'rgba(239,68,68,.3)' : 'rgba(59,130,246,.2)');
  ctx.strokeStyle = ringColor; ctx.lineWidth = 2;
  ctx.shadowColor = ringColor; ctx.shadowBlur = inside ? 8 : 4;
...
function animate() {
  ...
  if (radarMap && compassSupported) {
    ...
    if (mapEl) {
      ...
      var pins = mapEl.querySelectorAll('.map-branch-pin, .map-user-pin');
      for (var p = 0; p < pins.length; p++) {
        pins[p].style.transform = 'rotate(' + smoothHeading + 'deg)';
      }
    }
  }
...
function retryGPS() {
  ...
  navigator.geolocation.getCurrentPosition(function(p) {
    ...
    updateUI(dist); updateMapUser(userLat, userLon); setGPS('ready', T.gps_high_accuracy);
    document.querySelectorAll('.btn').forEach(function(b){ b.disabled = false; });
  }, function(e) {
    if (e.code === 1) { gpsPermissionDenied = true; showGPSModal(); }
...
function submitAttendance(type) {
  ...
  var sp = document.getElementById('spinner'); if (sp) sp.classList.add('show');
  hideMsg();
  function doPost(la, lo, ac) {
    ...
    .then(function(d){
      ...
      else { showMsg('error', d.message); btns.forEach(function(b){ b.disabled = false; }); }
    }).catch(function(){
      if (sp) sp.classList.remove('show');
      showMsg('error', T.msg_error_network);
...
function initRadarMap() {
  ...
  var brIcon = L.divIcon({
    className: '',
    html: '<div class="map-branch-pin"><img src="' + URLS.logoUrl + '"></div>',
    iconSize: [28,28], iconAnchor: [14,14]
  });
  radarBranchMarker = L.marker([CFG.workLat, CFG.workLon], {icon: brIcon}).addTo(radarMap);

  // Geofence circle around branch
  L.circle([CFG.workLat, CFG.workLon], {
    radius: CFG.geofenceRadius, color: 'rgba(59,130,246,.7)', fillColor: '#3B82F6', fillOpacity: 0.06, weight: 1.5, dashArray: '6,4'
  }).addTo(radarMap);
}
...
function playRadarSound() {
  try {
    ...
    ping(0,1200,0.5); ping(0.15,1800,0.3); ping(0.7,1200,0.5); ping(0.85,1800,0.3);
  } catch(e) {}
}
